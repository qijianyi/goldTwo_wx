...
1、系统icon采用iconfont来制作，在iconfont上生成css后，需要把 iconfont 中的 font-size 改为 40rpx
...
2、单文件上传（图片，视频）使用 this.$ajax() 且带固定参数;
```javascript
this.$ajax('', {
	query: {},
	file:'image', // video
...
3、多图片上传使用 this.$ajax() 且带固定参数，根据progress获取单张图片上传结果;
```javascript
this.$ajax('', {
	query: {},
	file:'image',
	count:8,
	progress: (issucc, json, i, max) => {},
	complete: (rsp,json) => {},
});
```
